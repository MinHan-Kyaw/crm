            ...
            if "userid" not in body or "atoken" not in body or "appid" not in body or "orgid" not in body or "domainid" not in body:
                ...
                if tokenreturn == "0":
                    ...
                    if len(stagelist) == 0: 
                        ...
                        reObj=dict()
                        stagelistreturn=list()
                        for i in range(len(stagelist)):
                            reObj['stageid']=stagelist[i][1]
                            reObj['name']=stagelist[i][2]
                            reObj['sort']=stagelist[i][5]
                            reObj_copy=reObj.copy()
                            stagelistreturn.append(reObj_copy)
                        forRes=dict()
                        forRes['returncode']="300"
                        forRes['list']=stagelistreturn
                        forRes['total']=len(stagelistreturn)
                        forRes['status']="Deal Stage List!"
                        response=forRes
                        return cb(200,response)
                elif tokenreturn == "1":
                    response = {
